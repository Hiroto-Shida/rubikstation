...
export const RecordPresenter = React.memo(({ timerState, ao5, ao12 }: Props) => {
      ...
      <>
        <Box component="div" sx={{ display: "flex", justifyContent: "center" }}>
          <Typography variant="h6">
            AO5: {ao5 === 0 ? "-" : `(${ao5_minutes}:${ao5_seconds}:${ao5_milliseconds})`}
          </Typography>
          <Typography variant="h6" sx={(theme) => ({ ml: theme.spacing(3) })}>
            A12: {ao12 === 0 ? "-" : `(${ao12_minutes}:${ao12_seconds}:${ao12_milliseconds})`}
          </Typography>
        </Box>
      </>
    )
